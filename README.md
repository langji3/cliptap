...
剪贴板历史和快捷片段统一使用“复制到当前剪贴板，再粘贴到原输入框”的操作。历史记录复用 Windows 原历史项，按原复制时间显示；使用旧条目不会将其作为新复制内容插入顶部，并记住选中位置，方便按顺序粘贴。新复制的内容仍显示在顶部。

可在**设置 → 唤醒快捷键**点击录入新组合，按 Esc 取消，或恢复默认 `Alt+空格`。支持 Ctrl/Alt 加字母、数字、空格或 F1–F11，可搭配 Shift。保存后立即生效并在下次启动时恢复；快捷键被占用或保存失败时保留原设置。托盘及面板提示同步显示新快捷键。

> 当前版本为 0.1.7。Windows 原生 WPF 界面；不依赖浏览器运行时，不上传剪贴板。安装版仅在用户主动检查或下载更新时访问 GitHub。
...
从列表收起后再次唤起默认进入剪贴板，并保留上次选中的历史条目；该条目已不存在时选中最新条目。编辑和设置页面暂时收起后继续当前页面。通过托盘打开时只复制；通过快捷键打开时记录原输入窗口，尝试恢复焦点后发送 Ctrl+V。没有有效目标、目标已关闭或焦点无法核验时，只复制并提示手动粘贴。操作系统不能保证识别所有应用内的可编辑控件。
...
- 列表仅显示片段标题和状态，悬停可查看触发词。敏感内容在编辑器中默认遮蔽，从面板粘贴无需二次确认。为敏感片段配置触发词后，输入完整触发词会直接展开，不另行确认。
...
- 选中片段后按 Enter 或单击即可粘贴到原输入框，敏感片段也无需二次确认。敏感内容仍附加 Windows 剪贴板历史/同步排除标记；不定时清除，也不在退出时清空剪贴板。
