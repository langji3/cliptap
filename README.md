...
> 当前版本为 0.1.2。Windows 原生 WPF 界面；不依赖浏览器运行时，不上传剪贴板。安装版仅在用户主动检查或下载更新时访问 GitHub。
...
也可点击“前往下载页”手动下载安装包。`v0.1.0` / `v0.1.1` 用户需先手动升级到 `v0.1.2`，之后才能使用一键更新。便携版不提供更新入口。卸载保留已保存片段和设置。
