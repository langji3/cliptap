...
按 **Ctrl + Alt + V** 唤起，左右切换剪贴板与快捷片段，上下选择，回车粘贴。也支持鼠标单击插入。用完即收起，平时留在托盘。

> 当前为 0.1 原型。Windows 原生 WPF 界面；不依赖浏览器运行时，不连接云端，不上传剪贴板。

支持 **浅色 / 深色 / 跟随系统**，默认跟随 Windows；**蓝 / 绿 / 紫** 三种主题色可独立组合。设置里切换即可预览，保存后记住选择；取消会恢复原主题和主题色。浮窗、编辑器、设置窗口及托盘标识统一切换。
...
3. 在输入框中按 `Ctrl+Alt+V` 唤起，选中内容并按回车或单击。
4. `→` 打开快捷片段，点击「新建片段」保存标题与内容；`F2` 编辑选中片段。
5. `Esc` 或点击外部收起。右键托盘可暂停记录、打开设置或退出。
...
每次唤起默认进入剪贴板并选中最新条目。通过托盘打开时只复制；通过快捷键打开时记录原输入窗口，尝试恢复焦点后发送 Ctrl+V。没有有效目标、目标已关闭或焦点无法核验时，只复制并提示手动粘贴。操作系统不能保证识别所有应用内的可编辑控件。
