...
触发词为 `!` 加 2–31 个小写字母、数字或下划线，末尾必须是字母或数字。区分大小写，不能重复或互为前缀。自动替换支持最多 2000 字符的单行内容，多行仍从面板粘贴。点击鼠标、切换焦点、移动光标、删除、组合键、输入法变化或超过 5 秒的输入间隔会取消匹配。片段保存、修改、删除后立即生效。
...
- 自动替换使用 Windows Unicode 输入，不写剪贴板，也不创建历史记录；后台只匹配已配置触发词的短前缀，不记录或上传日常键盘输入。监听在独立消息线程运行，不受面板绘制或磁盘保存阻塞。失败后暂停，托盘可恢复。
